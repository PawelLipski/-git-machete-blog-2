...
:Just recall last time when you had to review a giant like this:
...
The initial structure of branches in the demo (i.e., the contents of the definition file, excluding custom annotations that will be covered soon) is as follows:
...
To make yourself free from thinking over what to check out/rebase/push next, you can use `git machete traverse` -
a wizard that walks (or rather, `traverse`s) the branch dependency tree and suggests what needs to be done next to restore sync of branches with their parent branches and remotes.
...
Note if there were any conflicts during the rebase (or for that matter, any other failure during the traversal), `traverse` stops and lets your sort out the repository,
after which it can be simply re-run to pick up from where it finished.

After a successful rebase, `allow-ownership-link` was pushed to a remote (or more precisely, a _local remote_ since it resides on the local machine) repository and similar process happened again for `build-chain`.
For the sake of brevity it isn't included in the above screenshot, but if we were to continue the traversal,
then we'd encounter the branch `call-ws` which was aligned with `develop`, so rebase hasn't even been proposed.
Similiarly, `traverse` would later switch to `master` but wouldn't suggest to push or pull `master` since it was already in sync with its remote counterpart.
...
Each node of the tree (i.e. each git branch) is visited and possibly synced with its parent branch and/or remote before any of its children are visited.
This way of traversal makes more sense than, say, post-order or breadth-first search since you definitely want to put each branch `X` in sync with its parent branch first before syncing `X`'s children to `X` itself.
...
There's a couple of contexts where `git machete` can do this kind of guessing.

### `show up`, `go up`

The `sandbox-setup-2.sh` script added, among others, a branch `drop-constraint` that is not managed by git machete (i.e. not listed in `.git/machete`).
Let's assume for a moment that we're in an awkward situation of not remembering what branch it originally depended on and it's hard to find it out from `git log`.

So... `git machete` for the rescue!
...
Note that's in a way similar approach to how the fork point (useful for many commands like `reapply`, `slide-out` and `update` and covered in more detail in the first part of the blog) is inferred.
...
The stderr output warns that `drop-constraint` isn't present in `.git/machete` and suggests an inferred fallback (that's also printed to stdout to facilitate e.g. use in scripts).
If you take a look at the `sandbox-setup-2.sh`, `drop-constraint` was indeed based off `call-ws`, even though the branches are no longer in sync.
Similarly, `go up` subcommand checked out the inferred upstream.
...
Note that both in case of `update` and `add` the user has been explicitly asked to accept the git machete's guess.
...
`infer` simply performed upstream inference (just as for `show up` etc.) for every single local branch independently,
...
Also, if we changed the tree structure in the meantime (so that e.g. `develop` is a child of `call-ws` instead of the reverse) we can't expect `infer` to guess exactly the changed structure.
The inference is based on git reflogs and doesn't know anything about current or previous state of `.git/machete` file.

At this point one can ask a question: why then is the `.git/machete` definition file even needed since the upstreams could be always inferred on the fly?
The argument against such approach that is that `git` suite tools in general don't want everything to happen automagically/behind the scenes... and `git machete` strives to follow that philosophy.
The developer owns a sensible amount of control while still being provided with some hints when they're in doubt.
It seems much more reasonable than the reverse - inferring everything what's possible and only asking the human when the tool is in doubt.
