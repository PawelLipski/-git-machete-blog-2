...
Also, special thanks for the GitHub user [sprudent](https://github.com/sprudent) who raised an issue regarding crashes of git machete when run from a git submodule.
...
A tricky thing about this script is that it actually sets up two repos - one at `~/machete-sandbox` and another one at `~/machete-sandbox-remote`.
...
This will serve as a dummy remote repo for the one under `~/machete-sandbox`.
...
To make yourself free from thinking over what to check out/rebase/push next, you can turn to `git machete traverse` -
...
Note if there were any conflicts during the rebase (or for that matter, any other failure during the traversal), `traverse` would stop and let your sort out the repository,
...
Let's assume for a moment that we're in an awkward situation of not remembering what branch it originally depended on and it's hard to find that out from `git log`.
...
Also, if we changed the tree structure in the meantime (so that e.g. `develop` is a child of `call-ws` instead of the inverse) we can't expect `infer` to exactly guess the changed structure.
...
The argument against such approach that it isn't desirable that everything in the tool happens automagically/behind the scenes.
